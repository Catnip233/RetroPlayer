...
- 记忆每个视频的播放位置，并在首页与控制栏提供最近播放记录
- 播放结束后可按文件名顺序自动播放同文件夹的下一集
- 支持 0.5×–2× 倍速、字幕与音频延迟调整
- 可调整字幕字体、字号、垂直位置与描边粗细
- 播放窗口自动贴合视频真实显示比例，不再固定为 4:3 或在画面外留黑边
- 播放时鼠标静止 2 秒会自动隐藏底部控制栏与红黄绿窗口按钮，移动鼠标立即恢复
...
- 支持全局 Shader 强度；按住原画按钮或 `Tab` 可临时绕过效果，Shader 编译失败时自动回退
